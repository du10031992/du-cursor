namespace MepPanel.Core;

/// <summary>
/// MEPDB = entry. Con lai = sub-feature khoa/mo trong panel.
/// </summary>
public static class PluginFeatures
{
    public const string Entry = "MEPDB";
    public const string MepDb = Entry;

    public const string Cabinet2D = "MEPDBCABINET2D";
    public const string Cabinet3D = "MEPDBCABINET3D";
    public const string CabinetViews = "MEPDBCABINETVIEWS";
    public const string Config = "MEPDBCONFIG";
    public const string Duplicate = "MEPDBDUPLICATE";
    public const string Export = "MEPDBEXPORT";
    public const string Knowledge = "MEPDBKNOWLEDGE";
    public const string Power = "MEPDBPOWER";
    public const string RealRender = "MEPDBREALRENDER";
    public const string RealWiring = "MEPDBREALWIRING";
    public const string Render = "MEPDBRENDER";
    public const string Smoke = "MEPDBSMOKE";
    public const string Unfold = "MEPDBUNFOLD";
    public const string Update = "MEPDBUPDATE";
    public const string DeviceBlocks = "MEPDEVICEBLOCKS";
    public const string MepHvac = "MEPHVAC";
    public const string HvacConfig = "MEPHVACCONFIG";
    public const string HvacDraw = "MEPHVACDRAW";
    public const string HvacSmoke = "MEPHVACSMOKE";
    public const string SeLayer = "MEPSELAYER";
}
