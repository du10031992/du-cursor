...
public sealed class MepDrawingToolPanel : UserControl
{
    ...
    public MepDrawingToolPanel()
    {
        ...
        layout.Controls.Add(BuildGroup("Tủ điện / DB", new (string, string, Action)[]
        {
            ...
            ("Render bo tri tu -> PNG", PluginFeatures.Cabinet2D, () => Run(PluginFeatures.Cabinet2D, MepCabinetRenderService.RenderFromDrawing))
        }));

        layout.Controls.Add(BuildGroup("Minh hoa render PNG", new (string, string, Action)[]
        {
            ("Render he nuoc -> PNG", PluginFeatures.Water, () => Run(PluginFeatures.Water, MepPipeRenderService.RenderWater)),
            ("Render PCCC -> PNG", PluginFeatures.Smoke, () => Run(PluginFeatures.Smoke, MepPipeRenderService.RenderFire)),
            ("Render tu dien -> PNG", PluginFeatures.Cabinet2D, () => Run(PluginFeatures.Cabinet2D, MepCabinetRenderService.RenderFromDrawing))
        }));
